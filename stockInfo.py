...
def getHtmlFromUrl(url, utf8coding=False):
    try:
        ...
        return getHtmlFromUrl(url, utf8coding)
    return res
...
class StockUtils(object):
            ...
            if isinstance(obj, list) and len(obj) > 0:
                ...
                for item in obj:
                    ...
                    try:
                        ...
                        print 'parse error:%s, %s, %s, %s, %s' % (
                            code, incomeIncreaseByYear, profileIncreaseByYear, income, profit)
    ...
    @classmethod
    def getCommentNumberIn3MonthsForCode(cls, code):
        """3个月内评级"""
        ...
        if obj:
            ...
            if data and len(data):
                for item in data:
                    if item['emRatingName'] == u'增持' or item['emRatingName'] == u'买入' or \
                            item['lastEmRatingName'] == u'增持' or item['lastEmRatingName'] == u'买入':
                        ret.append(item)
        return len(ret)

    @classmethod
    def getRoeModelListOfStockForCode(cls, code):
        """价值投资股票信息"""
        ...
        if isinstance(ROEList, list) and len(ROEList) > 0:
            cList = []
            for item in ROEList:
                m = RoeModel(item['date'], item['tbjzcsyl'], item['gsjlrtbzz'], item['yyzsrtbzz'],
                             item['yyzsr'], item['kfjlr'], item['mll'], item['jll'], item['zcfzl'])
                cList.append(m)
            return cList
        else:
            return None
    ...
    def getRoeModelListOfStockInYearsForCode(self, code):
        ...
        if isinstance(ROEList, list) and len(ROEList) > 0:
            cList = []
            for item in ROEList:
                m = RoeModel(item['date'], item['jqjzcsyl'], item['gsjlrtbzz'], item['yyzsrtbzz'], item['yyzsr'],
                             item['kfjlr'], item['mll'], item['jll'], item['zcfzl'])
                cList.append(m)
            return cList
        else:
            return None
    ...
    def roeStringForCode(self, code, returnData=False):
        ...
        if li and len(li) > 0:
            for item in li:
                s += (u'季报:' + str(item.dateOfRoe)).ljust(15, ' ') + (u'净资产收益率:' + str(item.roe) + '%').ljust(15,
                                                                                                              ' ') + (
                             u'收入同比增长率:' + str(item.incomeRate) + '%').ljust(17, ' ') + (
                             u'净利润同比增长率:' + str(item.profitRate) + '%').ljust(18, ' ') + (
                             u'总收入:' + str(item.income)).ljust(12, ' ') + (u' 总利润:' + str(item.profit)).ljust(12,
                                                                                                              ' ') + (
                             u'毛利率:' + str(item.maolilv) + '%').ljust(13, ' ') + (
                             u'净利率:' + str(item.jinglilv) + '%').ljust(13, ' ') + (
                             u'资产负债率:' + str(item.zcfzl) + '%').ljust(13, ' ')
                s += '\n'
            return s
        else:
            return None
    ...
    def roeStringInYearsForCode(self, code):
        ...
        if li and len(li) > 0:
            for item in li:
                s += (u'年报:' + str(item.dateOfRoe)).ljust(15, ' ') + (u'净资产收益率:' + str(item.roe) + '%').ljust(15,
                                                                                                              ' ') + (
                             u'收入同比增长率:' + str(item.incomeRate) + '%').ljust(17, ' ') + (
                             u'净利润同比增长率:' + str(item.profitRate) + '%').ljust(18, ' ') + (
                             u'总收入:' + str(item.income)).ljust(12, ' ') + (u' 总利润:' + str(item.profit)).ljust(12,
        ...
        if obj and isinstance(obj, object):
            try:
                ...
                if holdings and len(holdings) > 0:
                    ...
                    holdingsCountRet = map(lambda x: 0 if x == '--' else (
                        float(x[0: -1]) * 10000 if '万' in x else float(x[0:-1]) * 10000 * 10000 if '亿' in x else float(
                            x)), holdingsCount)
                    jeRet = map(lambda x: 0 if x == '--' else (float(x[0: -1]) if '万' in x else float(x) / 10000), jes)
                    countRet = map(lambda x: 0 if x == '--' else (
                        float(x[0: -1]) * 10000 if '万' in x else float(x[0:-1]) * 10000 * 10000 if '亿' in x else float(
                            x)), counts)

                    if jeRet and len(jeRet) > 0:
                        if len(jeRet) == 1:
                            return jeRet[0], [jeRet[0], jeRet[0], jeRet[0]], [countRet[0], countRet[0], countRet[0]], [
                                holdingsCountRet[0], holdingsCountRet[0], holdingsCountRet[0]]
                        elif len(jeRet) == 2:
                            return jeRet[0], [jeRet[0], jeRet[1], jeRet[1]], [countRet[0], countRet[1], countRet[1]], [
                                holdingsCountRet[0], holdingsCountRet[1], holdingsCountRet[1]]
                        else:
                            return jeRet[0], [jeRet[0], jeRet[1], jeRet[2]], [countRet[0], countRet[1], countRet[2]], [
                                holdingsCountRet[0], holdingsCountRet[1], holdingsCountRet[2]]
...
def bussinessPercentString(code):
    ...
    if li and len(li) > 0:
        for model in li:
            s += model.bussinessName.ljust(13, ' ') + (u'收入:' + model.income).ljust(13, ' ') + (
                    u'利润:' + model.profit).ljust(13, ' ') + (u'收入占比:' + model.incomePercent).ljust(16, ' ') + (
                         u'利润占比:' + model.profitPercent).ljust(13, ' ')
            s += '\n'
    if len(s) > 0:
        return s
    return None
...
def mainMethod():
    ...
    for gd in ret:
        ...
        if companyInfo[6] != '-' and float(companyInfo[6]) > 0:
            print companyInfo[2], companyInfo[9].ljust(7, ' '), companyInfo[5], ' 增持: ', companyInfo[6], '增持后: ', \
                companyInfo[7], (
